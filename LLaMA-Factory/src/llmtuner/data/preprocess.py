...
def preprocess_dataset(
    ...
    def preprocess_supervised_dataset(examples: Dict[str, List[Any]], use_only_last_turn=False) -> Dict[str, List[List[int]]]:
        ...
        for query, response, history, system in construct_example(examples):
            ...
            encoded_multiturn = template.encode_multiturn(
                tokenizer, query, response, history, system
            )
            for turn_idx, (source_ids, target_ids) in enumerate(encoded_multiturn):
                ...
                labels += source_mask + ([IGNORE_INDEX] * len(target_ids) if use_only_last_turn and turn_idx < len(encoded_multiturn) - 1 else target_ids)
    ...
    def preprocess_packed_supervised_dataset(examples: Dict[str, List[Any]], use_only_last_turn=False) -> Dict[str, List[List[int]]]:
        # build inputs with format `<bos> X1 Y1 <eos> <bos> X2 Y2 <eos>`
        # and labels with format `<ignore> ... <ignore> Y1 <eos> <ignore> ... <ignore> Y2 <eos>`
        if use_only_last_turn:
            raise NotImplementedError(f"Not Implemented: use_only_last_turn = {use_only_last_turn}")
    ...
    def preprocess_pairwise_dataset(examples: Dict[str, List[Any]]) -> Dict[str, List[List[int]]]:
        ...
        for query, response, history, system in construct_example(examples):
            ...
            prompt_ids, chosen_ids = template.encode_oneturn(tokenizer, query, response[0].lstrip(' '), history, system)
            _, rejected_ids = template.encode_oneturn(tokenizer, query, response[1].lstrip(' '), history, system)
    ...
    if stage == "pt":
        ...
        preprocess_func = partial(preprocess_packed_supervised_dataset, use_only_last_turn=data_args.use_only_last_turn) if data_args.sft_packing else partial(preprocess_supervised_dataset, use_only_last_turn=data_args.use_only_last_turn)
